...
| Omitir la adición de usuarios al aprovisionar SQL | <ul> <li>`skipAddingSqlUser` propiedad in`.fx/config.<envName>.json`.</li> </ul> | [Omitir la adición de usuario para SQL base de datos](provision.md#skip-adding-user-for-sql-database). |
...
- 2. Actualice la propiedad del *manifiesto > appName > short* to `myapp(dev)`
...
- 6. Abra Teams de manifiesto de aplicación para el control remoto `templates/appPackage/manifest.remote.template.json`.
...
- 1. Abra Teams de manifiesto de aplicación para el control remoto `templates/appPackage/manifest.remote.template.json`.
