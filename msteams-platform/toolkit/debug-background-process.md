...
ms.locfileid: "63731893"
---
# <a name="debug-background-process"></a>Depurar procesos en segundo plano

El flujo de trabajo de depuración local implica los archivos `.vscode/launch.json` y `.vscode/tasks.json` para configurar el depurador en Visual Studio Code, el Visual Studio Code inicia los depuradores y el depurador de Microsoft Edge o Chrome inicia una nueva instancia del explorador de la siguiente manera:

1. El archivo `launch.json` configura el depurador en Visual Studio Code. 

2. Visual Studio Code ejecuta el `preLaunchTask` compuesto, `Pre Debug Check & Start All` en el archivo `.vscode/tasks.json`.
...
* Node.js, aplicable a los siguientes tipos de proyecto:
  ...
  |Extensión de mensajería | 10, 12, **14 (recomendado)**, 16 |
...
* Habilite la carga o instalación de prueba de aplicaciones personalizadas para el espacio empresarial del desarrollador. Si no es así, finaliza la depuración local.

* Ngrok versión binaria 2.3, aplicable a la extensión de bot y mensajería.  Si Ngrok no está instalado o la versión no coincide con el requisito, el kit de herramientas de Teams instala el paquete NPM de Ngrok `ngrok@4.2.2` en `~/.fx/bin/ngrok`. El paquete Ngrok NPM administra el binario Ngrok en `/.fx/bin/ngrok/node modules/ngrok/bin`.
...
* Paquetes NPM, aplicables a aplicaciones de pestañas, aplicaciones de bot, aplicaciones de extensión de mensajería y Azure Functions. Si NPM no está instalado, el kit de herramientas de Teams instala todos los paquetes NPM.

* Extensión de bot y mensajería. El kit de herramientas de Teams inicia Ngrok para crear un túnel HTTP para el bot y la extensión de mensajería.

* Los puertos disponibles, si los puertos de pestaña, bot, extensión de mensajería y Azure Functions no están disponibles, la depuración local finaliza.
  ...
  | --- | --- |
  | Tab | 53000 |
  | Bot, extensión de mensajería | 3978. |
  | Inspector de nodo para bot o extensión de mensajería | 9239. |
...
1. [ Registra y configura el bot](#registers-and-configures-bot): El kit de herramientas de Teams registra y configura el bot para la aplicación de extensión de mensajería o pestaña.
...
1. Expone una API.
    ...
    a. Configura el URI de id. de aplicación. Para la pestaña, `api://localhost/{appId}`. Para la extensión de bot o mensajería, `api://botid-{botid}`.
...
En la tabla siguiente se muestra la configuración de la autenticación como se indica a continuación:
  ...
  | Bot, extensión de mensajería | `https://ngrok.io/auth-end.html` | ND |
  
### <a name="registers-and-configures-bot"></a>Registra y configura el bot 

Para la aplicación de pestaña o la aplicación de extensión de mensajería:
...
Registra una aplicación de Teams en [Developer](https://dev.teams.microsoft.com/home) mediante la plantilla de manifiesto en `templates/appPackage/manifest.local.template.json`.
...
Puede ver las carpetas y los archivos del proyecto en el área explorador de Visual Studio Code después de que el kit de herramientas de Teams registre y configure la aplicación. En la tabla siguiente se enumeran los archivos de depuración locales y los tipos de configuración:

| Nombre de la carpeta| Contenido| Tipo de configuración de depuración |
| --- | --- | --- |
|  `.fx/configs/localSettings.json` | Archivo de configuración de depuración local | Los valores de cada configuración se generan y guardan durante la depuración local. |
|  `templates/appPackage/manifest.local.template.json` | Archivo de plantilla de manifiesto de aplicación de Teams para depuración local | Los marcadores de posición del archivo se resuelven durante la depuración local. |
...
|  `bot/.env.teamsfx.local` | Archivo de variables de entorno para el bot y la extensión de mensajería| Los valores de cada variable de entorno se generan y guardan durante la depuración local. |
