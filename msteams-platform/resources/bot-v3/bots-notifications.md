...
Microsoft Teams envía notificaciones al bot para los cambios o eventos que ocurren en ámbitos en los que el bot está activo. Puede usar estos eventos para desencadenar la lógica de servicio, como la siguiente:

* Desencadenar un mensaje de bienvenida cuando el bot se agrega a un equipo
* Consultar y almacenar en caché información de grupo cuando el bot se agrega a un chat de grupo
* Actualizar la información almacenada en caché sobre la pertenencia al equipo o la información del canal
* Quitar información almacenada en caché para un equipo si se quita el bot
...
Los eventos de teams y grupos, normalmente desencadenados fuera del tipo, tienen información de eventos de Teams adicional pasada como parte del objeto y, por lo tanto, el controlador de eventos debe consultar la carga de Teams y metadatos adicionales específicos de `conversationUpdate` `channelData` `channelData` `eventType` eventos.
...
El evento con el objeto en la carga se envía cuando se agrega un bot a un equipo o se agrega un nuevo usuario a un equipo en el que se ha `conversationUpdate` `membersAdded` agregado un bot. Microsoft Teams también agrega `eventType.teamMemberAdded` en el `channelData` objeto.
...
El evento con el objeto en la carga se envía cuando se quita el bot de un equipo o se quita un usuario de un equipo en el que `conversationUpdate` se ha agregado un `membersRemoved` bot. Microsoft Teams también agrega `eventType.teamMemberRemoved` en el `channelData` objeto. Al igual que con el objeto, debe analizar el objeto del identificador de aplicación del `membersAdded` bot para determinar quién se `membersRemoved` quitó.
