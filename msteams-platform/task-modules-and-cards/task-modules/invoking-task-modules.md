...
El siguiente código proporciona un ejemplo de datos adjuntos de tarjetas de bot de tarjeta adaptable cuando se invoca desde un bot:
...
| `BOT_APP_ID` | string | No | Si se especifica un valor para `completionBotId`, el objeto `result` se envía mediante un mensaje `task/submit invoke` al bot especificado. `BOT_APP_ID` debe especificarse como bot en el manifiesto de la aplicación, es decir, no se puede enviar a ningún bot. |
